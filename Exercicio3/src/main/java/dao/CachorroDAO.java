...
public class CachorroDAO {
	...
	public boolean inserirCachorro(model.Cachorro cachorro) {
		boolean status = false;
		try {  
	...
	public boolean atualizarCachorro(model.Cachorro cachorro) {
		boolean status = false;
		try {  
			Statement st = conexao.createStatement();
	...
	public model.Cachorro[] getCachorros() {
		model.Cachorro[] cachorros = null;
		...
		try {
			...
			ResultSet rs = st.executeQuery("SELECT * FROM cachorro");		
	         if(rs.next()){
	             rs.last();
	             cachorros = new model.Cachorro[rs.getRow()];
	             rs.beforeFirst();

	             for(int i = 0; rs.next(); i++) {
	                cachorros[i] = new model.Cachorro(rs.getInt("codigo"), rs.getString("nome"), 
	...
	public model.Cachorro[] getMachos() {
		model.Cachorro[] cachorros = null;
